...
void run_cpu_gray_test(FULLImage* himage, Image img_in, char *out_filename)
{
    ...
    double b = 1.0;
    double s = 0.4; 
    pixel_t * off = paris_llf(img_in.w,img_in.h,img_baseaddr,a,b,s);
    
    //printf("offset is : %d\n",off);
    img_obuf.w = img_in.w;
    img_obuf.h =img_in.h;
    //img_obuf.img = &img_baseaddr[off];
    img_obuf.img  = off;
    printf("Running local laplacian filter finished with alpha %f beta %f sigma %f .\n",a,b,s);
